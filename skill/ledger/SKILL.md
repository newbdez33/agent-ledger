...
2. **Record each movement** with `--json`, a `--ref`, and for positions the same `--group` on every leg plus `--meta`.
...
| resolution payout without a tx | `settle:<market>` |
...
Pass `--ts` with the venue's event time (RFC 3339 or `YYYY-MM-DD`); omit it only for something happening now.
...
- `ledger pnl --by meta:strategy` buckets by a meta key; entries without the key land in the `null` bucket. Conventional keys, all strings: `market`, `side`, `price`, `shares`, `strategy`, `venue`.
...
Shapes: `add` → `{entry, balance, duplicate}`; `balance` → `{accounts:[...]}`; `history` → `{entries:[... balance_after]}`; `group` → `{entries, net}`; `pnl` → `{accounts:[{rows:[{bucket, trades, settlements, fees, adjustments, other, net}]}]}`; `reconcile` → `{snapshot, adjustment|null}`.
...
- Reconciling with an explicit `--ts` earlier than entries you already booked. The book as of that time excludes them, and the adjustment lands in the past. Use the time you observed the balance, never earlier than your latest entry.
