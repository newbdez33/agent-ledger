...
3. **Reconcile once per fetched balance.** `reconcile` compares observed with the book **as of `--ts`**, the same number `balance --at <ts>` shows, and records a snapshot carrying the diff. It posts nothing else. In a live loop a nonzero diff is usually fills you have not booked yet: `add` them with their refs and reconcile again. Only a diff you have explained is a discrepancy; then repeat the call with `--adjust` and it posts an `adjustment` at that time. `--dry-run` reports observed, book and diff and writes nothing. Pass the read time as `--ts` whenever you know it: the comparison is on entry `ts`, not `recorded_at`, so a correction dated before the read counts even if you wrote it afterwards; the snapshot records when the venue was read; and a retry with the same `--ts`, observed and source is a duplicate (`"duplicate": true`, nothing written) instead of a second snapshot. Without `--ts` it uses now or the latest booked entry, whichever is later, so the whole book counts, and every retry is a new snapshot.
   `ledger reconcile poly-usdc --observed 123.6075 --source polygon-rpc --ts 2026-09-06T03:12:45Z --json`
   A historical statement (month-end, a past snapshot) gets its own `--ts` and never `--adjust`: an adjustment there lands in the past and shifts every later balance. Rule of thumb: `--adjust` only for the newest balance you hold, and only once `add` cannot explain the diff.
...
- Adjusting away a diff in a live loop. A nonzero diff is usually unbooked fills: `add` them with their refs and reconcile again; `--adjust` is for a discrepancy you have explained.
- Retrying a `reconcile` without `--ts`. Each run is a new observation time and adds a snapshot; with the read time as `--ts` the retry is a duplicate.
