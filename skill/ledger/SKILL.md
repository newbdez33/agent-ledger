...
4. **Read**: `balance [account]`, `history <account> [--limit 0]` (default: the 50 most recent), `group <id>`, `pnl [account] --by total|day|week|month|group|meta:<key> [--marks marks.json]`, `snapshots <account>`, `export <account> --format csv|json`.
...
Round to the account's decimals before posting; more decimals than the account allows are rejected (`precision_exceeded`) unless they are zeros, never rounded.
...
An entry is `{id, account, currency, ts, recorded_at, kind, amount, ref, memo, actor, group, meta, reverses_id, reversed_by}`, the same object everywhere; `history` and `export --format json` add `balance_after` and list oldest `ts` first, and CSV uses the same column names. `export --format json` is one object while `import` reads one entry per line, so moving a book is `ledger --db old.db export poly-usdc --format json | jq -c '.entries[]' | ledger --db new.db import --dry-run`, then again without `--dry-run`: each exported entry object is a valid import line, groups included, and a file written by 0.1 with `group_id` loads too. Create the target account first with the same currency and decimals; import checks neither. An import line needs `account`, `amount` (a string) and `kind`; `ref`, `ts`, `group`, `memo`, `meta`, `actor` are optional, and every other key is ignored without a warning, so a misspelt key silently drops that value. Lines without `ref` are not replay-safe. A line's `actor` beats `LEDGER_ACTOR`. All lines are written or none, and the error names the line. `transfer` and `reversal` lines are rejected.
Shapes: `add` → `{entry, balance, duplicate}` where `balance` is the account's current balance (on a duplicate too); `show` → `{entry, balance}`, again the current balance, not the entry's running balance; `account add` → `{account, duplicate}`; `reverse` → `{entries:[reversal, …]}`; `import` → `{imported, duplicates, dry_run, entries:[entry, …]}`; `balance` → `{accounts:[{account, currency, balance, entries, last_ts, last_reconciled_at}]}` where `last_reconciled_at` is the latest snapshot's as-of `ts`, not when it ran, and `balance <account> [--at]` → `{account, currency, balance, at}`; `history` and `export --format json` → `{account, currency, entries:[entry + balance_after]}`; `group` → `{entries, net}`; `pnl` → `{accounts:[{rows:[{bucket, trades, settlements, fees, adjustments, other, net, open_value, mtm}]}]}` where `bucket` is `null` both for the no-value bucket and for the single `--by total` row (the table prints `null` and `total`), and `open_value` is `null` (a blank cell in the table) with `mtm` equal to `net` unless `--marks` valued a group in that row; `reconcile` → `{snapshot, adjustment|null, duplicate, dry_run}` where `snapshot.id` is `null` on a dry run; `snapshots` → `{account, snapshots:[{id, account, ts, observed, book, diff, adjustment_entry_id, source}]}` oldest first, last row is the latest, `ts` being the as-of time; nothing records when a reconcile ran.
